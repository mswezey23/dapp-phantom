...
// save common tx data
private.saveCommon = function (trs, cb) {

	library.logger.trace("logic.transaction.saveCommon", trs)
	...
	modules.api.sql.insert({
		...
		values: {
			...
			blockId: trs.blockId,
			token: trs.token == constants.defToken ? null : trs.token
		}
	}, cb);
}

// save special (type-dependent) tx data
private.saveSpecial = function (trs, cb) {

	library.logger.trace("logic.transaction.saveSpecial", trs)

	private.types[trs.type].save(trs, cb);
}

// saves trs to database
// what: 1: common data
// what: 2: type dependent data
// what: 3: both data (default)
Transaction.prototype.save = function (trs, cb, what) {

	what = what || 3;

	library.logger.trace("logic.transaction.save (" + what + ")", trs)

	if (!private.types[trs.type])
		return setImmediate(cb, "Unknown transaction type " + trs.type);

	if (what & 1)
		private.saveCommon(trs, function(err) {
			if (err)
				return cb(err);
		});

	if (what & 2)
		private.saveSpecial(trs, function(err) {
			if (err)
				return cb(err);
		});
...
Transaction.prototype.normalize = function (tx, cb) {
	...
	library.logger.trace("logic.transaction.normalize", tx)
...
Transaction.prototype.dbRead = function (row) {
	...
	var trs = {
		...
		token: row.t_token || constants.defToken,
		asset: {}
	};
