...
WithdrawalTransfer.prototype.apply = function (trs, sender, cb, scope) {
	modules.blockchain.accounts.mergeAccountAndGet({
		...
		balance: constants.defTokenObj(-(trs.amount + trs.fee))
	}, cb, scope);
}
...
WithdrawalTransfer.prototype.applyUnconfirmed = function (trs, sender, cb, scope) {
	...
	if (sender.u_balance[constants.defToken] < sum) {
		return cb("Account does not have enough " + constants.defToken);
	}

	modules.blockchain.accounts.mergeAccountAndGet({
		address: sender.address,
		u_balance: constants.defTokenObj(-(trs.amount + trs.fee))
	}, cb, scope);
}
...
WithdrawalTransfer.prototype.onBind = function (_modules) {
	...
	modules.logic.transaction.attachAssetType(trsTypes.WITHDRAWAL, self);
}
