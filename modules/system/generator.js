...
Generator.prototype.onBind = function (_modules) {
	...
	modules.api.dapps.getGenesis(function (err, res) {
		if (err) {
			return library.logger.fatal("Failed to get genesis block", err)
		}

		var executor = modules.blockchain.accounts.getExecutor();

		if (!executor) {
			return library.logger.warn("Secret is null")
		}
		...
		library.logger.warn(JSON.stringify(genesisBlock, null, 2))
	});
}
