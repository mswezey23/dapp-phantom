...
function Blocks(cb, _library) {
	...
	try {
		...
		library.logger.fatal("Failed to load genesis.json");
	}
	...
	self.getBlock(function (err, previousBlock) {
		...
		async.eachSeries(oldLastBlock.transactions.reverse(), function (transaction, cb) {
			async.series([
				function (cb) {
					fee += transaction.fee;
					modules.blockchain.transactions.undoTransaction(transaction, cb);
				}, function (cb) {
			...
			modules.blockchain.accounts.undoMerging({
				...
				balance: constants.defTokenObj(fee)
			}, function (err) {
...
private.verify = function (block, cb, scope) {
	...
	modules.api.blocks.getBlock(block.pointId, function (err, liskBlock) {
		if (err || !liskBlock) {
			return cb("Referenced Block (mainchain) not found: " + block.pointId);
		}
...
private.rollbackUntilBlock = function (block, cb) {
	modules.api.sql.select({
		...
		if (!err && found.length) {
			library.logger.log("Blocks#rollbackUntilBlock", found);
...
private.processBlocks = function (blocks, cb, scope) {
...
private.cleanProcess = function (blockObj, cb, scope) {
	...
	modules.blockchain.transactions.getUnconfirmedTransactionList(true, function (err, list) {
		async.eachSeries(list, function (transaction, cb) {
			...
			if (err) {
				library.logger.warn("Failed to undo transactions: " + err.toString());
				cb(err);
			} else {
				private.processBlock(blockObj, function (err) {
					if (err) {
						library.logger.warn("Failed to process block", err);
					}
...
// same as saveBlock, but in batch
Blocks.prototype.saveBatchBlock = function (blocks, cb) {

	// prepare generic block and transaction data
	var blocks_row = [];
	...
	for (var i = 0; i < blocks.length; i++) {
		...
		for (var n = 0; n < b.transactions.length; n++) {
			var trs = b.transactions[n];
	...
	modules.api.sql.batch({
		...
		modules.api.sql.batch({
			...
			// save special transaction data
			transactions.forEach(function (trs) {
				modules.logic.transaction.save(trs, function (err) {
					if (err)
						return cb(err);
					}, 2);		//  type dependent trs data only
			})
...
Blocks.prototype.deleteBlocksBefore = function (block, cb) {
	async.whilst(
		...
		function (next) {
			library.logger.log("Blocks#popLastBlock", private.lastBlock.height);
...
// ensure a defined transaction sequence
private.sortTransactions = function (transactions) {
	if (transactions.length>1)
		transactions.sort( function(a, b) { return a.id.localeCompare(b.id); });
	return transactions;
	};
...
Blocks.prototype.createBlock = function (executor, timestamp, point, cb, scope) {
	modules.blockchain.transactions.getUnconfirmedTransactionList(false, function (err, unconfirmedList) {
		...
		private.sortTransactions(unconfirmedList);
...
Blocks.prototype.applyBlock = function (block, cb, scope) {
	...
	private.sortTransactions(block.transactions);
	...
	async.eachSeries(block.transactions, function (transaction, cb) {
		...
		if (err) {
			async.eachSeries(block.transactions, function (transaction, cb) {
				...
				if (undoErr) {
					library.logger.warn(undoErr.toString());
					process.exit(0);
				}
				cb(err);
			});
			...
			async.eachSeries(block.transactions, function (transaction, cb) {
				modules.blockchain.transactions.applyTransaction(transaction, function (err) {
					if (err) {
						library.logger.warn("Failed to apply transactions: " + transaction.id);
				...
				if (err) {
					...
					modules.blockchain.accounts.mergeAccountAndGet({
						...
						balance: constants.defTokenObj(fee),
						u_balance: constants.defTokenObj(fee)
					}, function (err) {
						if (!err) {
...
Blocks.prototype.loadBlocksPeer = function (peer, cb, scope) {
	var offset = 0, limit = 100, count = 0, lastBlock = scope.lastBlock.height;
	var blocks = [];

	library.logger.log("Load blocks after:", lastBlock)

	modules.api.transport.getPeer(peer, "get", "/blocks/count", {}, function (err, res) {

		if (err || !res.body || !res.body.success) {
			return cb(err);
		}

		count = res.body.response;
		library.logger.warn("Blocks " + count);
		async.until(
			function ()	{
				return (count<offset)	|| (offset>10000);
			},
			function (cb) {
				library.logger.warn("Current " + (lastBlock+offset));

				modules.api.transport.getPeer(peer, "get", "/blocks/after", { lastBlockHeight: lastBlock, offset: offset, limit: limit}, function (err, res) {
					if (err || !res.body || !res.body.success) {
						return cb(err);
					}

					blocks = blocks.concat(res.body.response);
					offset += limit;
					cb(null);
				}, scope);		// modules.blockchain.blocks.loadBlocksPeerOffset
			},
			function (err) {
				if (err) {
					return setImmediate(cb, err);
				...
				private.processBlocks(blocks, cb, scope);
			}
		)		// async.until
	});		// modules.api.transport.getPeer(peer, "get", "/blocks/count"
}
...
Blocks.prototype.getBlocks = function (cb, query) {
  query.lastBlockHeight = 0;
  return self.getBlocksAfter(cb, query);
}

Blocks.prototype.getBlocksAfter = function (cb, query) {

  var orderBy = OrderBy((query.orderBy || 'height:asc'));
  if (orderBy.error)
    return setImmediate(cb, orderBy.error);

  var order = orderBy.sortMethod === 'DESC' ? -1 : 1;
  
  query.lastBlockHeight = query.lastBlockHeight || 0;
  query.limit           = (!query.limit || query.limit > 100) ? 100 : query.limit,
  query.offset          = query.offset || 0,
    
  // the query returns multiple records per block if there are more then 1 tx per block
  // these records will be combined in readDbRows later
  // => we cannot use offset/limit clause as usual
  self.count( function(err, blockcount) {
    if (err)
      return cb(err);
    
    if (order>0)
      {
      query.lastBlockHeight += query.offset;
      if (query.lastBlockHeight+query.limit < blockcount)
        blockcount = query.lastBlockHeight+query.limit;
      }
    else
      {
      blockcount -= query.offset;
      if (query.lastBlockHeight < blockcount-query.limit)
        query.lastBlockHeight = blockcount-query.limit;
      }
      
    var request = extend({}, library.scheme.selector["blocks"], {
      fields:     library.scheme.aliasedFields,
      condition:  { $and: [ { "b.height": {$gt: query.lastBlockHeight} }, { "b.height": {$lt: blockcount+1} } ] },
      sort:       {}
    });
    request.sort[orderBy.sortField] = order;
    
    modules.api.sql.select(request, library.scheme.types, function (err, blocks) {
      if (err)
        return cb(err);
      
      var blocks_ = self.readDbRows(blocks);
      cb(err, blocks_);
    });
  });
}
...
Blocks.prototype.onMessage = function (query) {
	if (query.topic == "block" && private.loaded) {
		library.sequence.add(function (cb) {
			...
			// library.logger.log("check", block.prevBlockId + " == " + private.lastBlock.id, block.id + " != " + private.lastBlock.id)
			if (block.prevBlockId == private.lastBlock.id && block.id != private.lastBlock.id && block.id != private.genesisBlock.id) {
				private.cleanProcess(block, function (err) {
					if (err) {
						library.logger.warn("Blocks#cleanProcess error", err);
					}
					cb(err);
				})
			} else {
...
Blocks.prototype.onBind = function (_modules) {
	...
	modules.api.sql.select({
		...
		if (err) {
			library.logger.warn("Failed to get genesis block", err)
			process.exit(0);
		}
		if (!found.length) {
			self.saveBlock(private.genesisBlock, function (err) {
				if (err) {
					library.logger.warn("Failed to save genesis block", err.toString());
					process.exit(0);
				} else {
