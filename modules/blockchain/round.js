...
private.loop = function (point, cb) {
	modules.blockchain.accounts.getExecutor(function (err, executor) {
		...
		if (!private.loaded) {
			library.logger.debug("loop: Blockchain syncing", "exit: syncing");
		...
		if (currentSlot == slots.getSlotNumber(lastBlock.timestamp)) {
			library.logger.debug("loop", "exit: lastBlock is in the same slot");
			return setImmediate(cb);
		}

		var currentBlockData = private.getState(executor, point.height);
		if (currentBlockData === null) {
			library.logger.debug("loop: No delegate", "exit: skipping slot");
			return setImmediate(cb);
		}
		...
		library.sequence.add(function (cb) {
			...
			if (err) {
				library.logger.warn("Failed to generate block", err);
			} else {
				var lastBlock = modules.blockchain.blocks.getLastBlock();
				library.logger.log("New dapp block id: " + lastBlock.id + " height: " + lastBlock.height + " via point: " + lastBlock.pointHeight);
			}
			cb(err);
		})
	});
...
Round.prototype.onMessage = function (query) {
	if (query.topic == "point" && private.loaded) {
		...
		private.loop(block, function (err) {
			if (err) {
				library.logger.warn("loop: error", err)
			}
		});
	}
}
