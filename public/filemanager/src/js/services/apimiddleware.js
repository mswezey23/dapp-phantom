    ...
    angular.module('FileManagerApp').service('apiMiddleware', ['$window', 'fileManagerConfig', 'apiHandler', 
        ...
        ApiMiddleware.prototype.getFileList = function(files) {
            return (files || []).map(function(file) {
                return file && {'path': file.model.fullPath(), 'hash': file.model.hash}; // We also need te hash for IPFS
            });
        };
        ...
        ApiMiddleware.prototype.rename = function(item) {
            var itemPath = this.getFileList([item]); // this.getFilePath(item);
        ...
        ApiMiddleware.prototype.keyValue = function(key) {
        ...
        ApiMiddleware.prototype.resolve = function(name) {
            return this.apiHandler.resolve(fileManagerConfig.resolveUrl, name);
        };	
        ...
        ApiMiddleware.prototype.createFolder = function(item) {
            ...
            return this.apiHandler.createFolder(fileManagerConfig.createFolderUrl, path, content);
        };
